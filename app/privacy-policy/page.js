...
export default function PrivacyPolicy() {
  ...
  useEffect(() => {
    axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/content`)
      .then(r => {
        const item = r.data.find(c => c.section_name === 'privacy_policy')
        if (item?.content) setContent(item.content)
        else setContent('Privacy policy content will be added soon.')
      })
      .catch(() => setContent('Privacy policy content will be added soon.'))
  }, [])
    ...
    <>
      ...
      <section style={{ minHeight: '100vh', background: '#091509', padding: '4rem 2rem' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <h1 style={{ color: '#c9911a', fontSize: '2rem', marginBottom: '2rem' }}>Privacy Policy</h1>
