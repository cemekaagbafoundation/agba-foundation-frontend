...
export default function Hero() {
  const [heroData, setHeroData] = useState({ image_url: '', title: 'Chief Emeka Agba Foundation' })
    ...
    <section style={{
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <Link href="/apply" style={{ background: '#c9911a', color: '#061209', padding: '0.9rem 2rem', borderRadius: '6px', fontWeight: 'bold', fontSize: '0.95rem' }}>Apply for Training</Link>
          <Link href="/donate" style={{ border: '2px solid #c9911a', color: '#c9911a', padding: '0.9rem 2rem', borderRadius: '6px', fontWeight: 'bold', fontSize: '0.95rem' }}>Make a Donation</Link>
        </div>
      </div>

      <div style={{ width: '100%', aspectRatio: '4/3', borderRadius: '14px', overflow: 'hidden', background: '#0f2d16', border: '1px solid #1a4a20' }}>
        {heroData.image_url ? (
          <img src={heroData.image_url} alt={heroData.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
        ) : (
